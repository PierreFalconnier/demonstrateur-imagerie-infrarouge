...
def detect_one(model, orgimg, device):
    ...
    if orgimg.ndim == 2 : orgimg = np.stack((orgimg,)*3, axis=-1)  # convertion en rbg
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights_IR', nargs='+', type=str, default='weights/yolov5n_face.pt', help='model.pt path(s)')
    parser.add_argument('--weights_RGB', nargs='+', type=str, default='weights/yolov5n-face.pt', help='model.pt path(s)')
    parser.add_argument('--img-size', type=int, default=640, help='inference size (pixels)')
    opt = parser.parse_args()
    print(opt)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_IR = load_model(opt.weights_IR, device)
    model_RGB = load_model(opt.weights_RGB, device)
    ...
    while True:
        try:
            ...
            img_arr = detect_one(model_IR, img_arr  ,device)

            # RBG
            ret, orgimg = cap.read()
            orgimg = detect_one(model_RGB, orgimg  ,device)

            # Affichage
            cv2.imshow('image',np.concatenate((img_arr, orgimg), axis=1))
